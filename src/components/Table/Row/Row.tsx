...
const Row: FC<RowProps> = ({ cell, isSubCells, subCells, deleteRow }) => {
	...
	return (
		<>
			<div
				className={`row_block  ${rowDelete ? 'delete_row' : ''}`}
				onClick={check}
			>
				<div
					ref={rowRef}
				...
				>
					...
					{!isSubCells ? (
						<span onClick={deleteClick} className='action delete'>
							x
						</span>
					) : (
						<span
							onClick={hidingSub}
							className={`action  ${hidingSubRow ? 'close' : 'open'} ${
								!count && 'toggle'
							}`}
						>
							↓
